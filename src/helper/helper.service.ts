...
export class HelperService {
    ...
    async isObjectId(id: string): Promise<boolean> {
       const regex = /^(?=[a-f\d]{24}$)(\d+[a-f]|[a-f]+\d)/i;
       return regex.test(id);
    }

    async randomReference(prefix?: string,count:number=10): Promise<string> {
        const timestamp: string = `${new Date().valueOf()}`;
        const randomString: string = await this.randomString(count);
    ...
    async jwtCreateToken(
        ...
        return this.jwtService.sign(payload, {
            secret:
                ...
                this.configService.get<string>('helper.jwt.secretKey'):this.configService.get<string>('helper.jwt.secretKey'),
            expiresIn:
                options? options?.expiredIn ||
                this.configService.get<string>('helper.jwt.expirationTime'):this.configService.get<string>('helper.jwt.expirationTime'),
            notBefore:
