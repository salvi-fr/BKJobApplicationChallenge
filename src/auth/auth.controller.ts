...
export class AuthController {
    ...
    @Response('auth.login', ENUM_STATUS_CODE_SUCCESS.LOGIN_SUCCESS)
    async login(
        ...
        if (!user) {
            ...
            throw new ErrorHttpException(
                ENUM_STATUS_CODE_ERROR.AUTH_USER_NOT_FOUND_ERROR
            );
        }
        ...
        if (!validate) {
            ...
            throw new ErrorHttpException(
                ENUM_STATUS_CODE_ERROR.AUTH_PASSWORD_NOT_MATCH_ERROR
            );
        }
    ...
    @AuthJwtRefreshGuard()
    @HttpCode(HttpStatus.OK)
    @Response('auth.refresh',ENUM_STATUS_CODE_SUCCESS.LOGIN_SUCCESS)
